...
需要快照（Snapshot）的场景：保存文件的一个瞬时状态为另一个文件，用于备份或回滚。

快照的机制：**COW**（Copy On Write，写时复制）

- master**回收**对应chunk的**租约**，停止对应chunk的所有写入；
- **拷贝**一份文件的**元数据**并命名为快照文件（快照文件元数据仍指向原文件的chunk）；
- 增加文件所有chunk的**引用计数**；
- master正常授予租约，允许对chunk进行写入；
- 下次对文件进行修改时，发现其chunk的引用计数大于1，修改时会先拷贝一份新的chunk，然后在新chunk上写入。